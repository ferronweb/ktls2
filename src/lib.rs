...
impl CompatibleCiphers {
  ...
  /// List compatible ciphers. This listens on a TCP socket and blocks for a
  /// little while. Do once at the very start of a program. Should probably be
  /// behind a lazy_static / once_cell. This function uses Monoio
  #[cfg(any(test, feature = "monoio"))]
  pub async fn new_monoio() -> io::Result<Self> {
    ...
    // Accepted conns of ln
    let mut accepted_conns: SmallVec<[TcpStreamPoll; 12]> = SmallVec::new();

    let accept_conns_fut = async {
      loop {
        if let Ok((conn, _addr)) = ln.accept().await {
          accepted_conns.push(conn.into_poll_io()?);
        }
      }
    };
  ...
  async fn test_ciphers(
    ...
    accept_conns_fut: impl Future<Output = Result<(), std::io::Error>>,
  ) -> io::Result<()> {
