...
async fn server_test_inner(cipher_suite: KtlsCipherSuite, flavor: ServerTestFlavor) {
  ...
  let mut server_config = ServerConfig::builder_with_provider(single_suite_provider(cipher_suite))
  ...
  let jh = monoio::spawn(
    async move {
      ...
      let mut stream = acceptor
        .accept(stream.into_poll_io().unwrap())
        .await
        .unwrap();
      debug!("Completed TLS handshake");
