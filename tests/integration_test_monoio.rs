...
use ktls2::{CorkStream, KtlsCipherSuite, KtlsCipherType, KtlsVersion};
...
use monoio::{io::IntoPollIo, net::{TcpListener, TcpStream}};
...
async fn compatible_ciphers() {
    let cc = ktls2::CompatibleCiphers::new_monoio().await.unwrap();
...
async fn server_test_inner(cipher_suite: KtlsCipherSuite, flavor: ServerTestFlavor) {
    ...
    let ln = TcpListener::bind("[::]:0").unwrap();
    ...
    let jh = monoio::spawn(
        async move {
            ...
            let stream = SpyStream(stream.into_poll_io().unwrap(), "server");
            ...
            let mut stream = ktls2::config_ktls_server(stream).await.unwrap();
    ...
    let mut stream = tls_connector
        .connect("localhost".try_into().unwrap(), stream.into_poll_io().unwrap())
        .await
        .unwrap();
...
async fn client_test_inner(cipher_suite: KtlsCipherSuite, flavor: ClientTestFlavor) {
    ...
    let jh = monoio::spawn(
        async move {
            ...
            debug!("Accepted TCP conn from {}", addr);
            let mut stream = acceptor.accept(stream.into_poll_io().unwrap()).await.unwrap();
            debug!("Completed TLS handshake");
    ...
    let stream = TcpStream::connect(addr).await.unwrap();
    let stream = CorkStream::new(stream.into_poll_io().unwrap());
    ...
    let stream = ktls2::config_ktls_client(stream).await.unwrap();
